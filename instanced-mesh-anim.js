AFRAME.registerComponent('instanced-mesh', {
    schema: {
        ...
        enableAnimationMixers: { type: 'boolean', default: false }
    },
    ...
    init: function () {
        ...
        // Animation mixers for handling complex animations
        this.animationMixers = new Map(); // Map of member entity ID to AnimationMixer
        this.animationClips = new Map(); // Map of member entity ID to animation clips
        this.animationActions = new Map(); // Map of member entity ID to active actions
    ...
    memberAdded: function (event) {
        ...
        // Initialize animation mixer if enabled and member has animation clips
        if (this.data.enableAnimationMixers) {
            const memberComponent = member.components['instanced-mesh-member'];
            if (memberComponent && memberComponent.data.animationClips) {
                this.initAnimationMixer(member, memberComponent.data.animationClips);
            }
        }
    ...
    memberRemoved: function (event) {
        ...
        // Clean up animation mixer for this member
        this.removeAnimationMixer(event.detail.member.id);
    ...
    // to be up-to-date.
    prerender: function () {
        if (!this.autoMode && this.membersToUpdate.size === 0 && this.animationMixers.size === 0) return;
        ...
        // Update animation mixers
        if (this.data.enableAnimationMixers && this.animationMixers.size > 0) {
            const deltaTime = this.clock ? this.clock.getDelta() : 0.016; // fallback to ~60fps

            this.animationMixers.forEach((mixerData, memberId) => {
                const { mixer, target, member } = mixerData;

                // Update the mixer
                mixer.update(deltaTime);

                // Apply animation results to member object3D
                member.object3D.position.copy(target.position);
                member.object3D.rotation.copy(target.rotation);
                member.object3D.scale.copy(target.scale);
                member.object3D.updateMatrix();

                // Update instanced mesh matrices
                const memberIndex = this.orderedMembersList.findIndex(m => m.id === memberId);
                if (memberIndex !== -1) {
                    this.updateMatricesFromMemberObject(member.object3D, memberIndex);
                }
            });
        }
        ...
        if (this.autoMode) {
            ...
            list.forEach((member, index) => {
                const memberComponent = member.components['instanced-mesh-member'];
                if (memberComponent && memberComponent.animation && !this.animationMixers.has(member.id)) {
                    // Apply simple animation logic for non-mixer animations
    ...
    // Animation Mixer Support Methods
    initAnimationMixer: function (member, animationClips) {
        if (!this.data.enableAnimationMixers) return null;

        const memberId = member.id;

        // Create a dummy object for the mixer to animate
        const animationTarget = new THREE.Object3D();
        animationTarget.position.copy(member.object3D.position);
        animationTarget.rotation.copy(member.object3D.rotation);
        animationTarget.scale.copy(member.object3D.scale);

        const mixer = new THREE.AnimationMixer(animationTarget);

        this.animationMixers.set(memberId, {
            mixer: mixer,
        ...
        if (animationClips && animationClips.length > 0) {
            this.animationClips.set(memberId, animationClips);
            ...
            this.animationActions.set(memberId, actions);
        }

        return mixer;
    },

    playAnimation: function (memberId, clipName, options = {}) {
        const actions = this.animationActions.get(memberId);
        if (!actions || !actions[clipName]) {
            console.warn(`Animation clip '${clipName}' not found for member ${memberId}`);
            return;
        }

        const action = actions[clipName];

        // Stop other animations if not blending
        if (!options.blend) {
    ...
    stopAnimation: function (memberId, clipName = null) {
        const actions = this.animationActions.get(memberId);
        if (!actions) return;
        ...
        if (clipName && actions[clipName]) {
            ...
            // Stop all animations for this member
            Object.values(actions).forEach(action => action.stop());
        }
    },

    removeAnimationMixer: function (memberId) {
        if (this.animationMixers.has(memberId)) {
            const mixerData = this.animationMixers.get(memberId);
            mixerData.mixer.stopAllAction();
            this.animationMixers.delete(memberId);
        }
        this.animationClips.delete(memberId);
        this.animationActions.delete(memberId);
    },
...
AFRAME.registerComponent('instanced-mesh-member', {
    schema: {
        ...
        colors: { type: 'array' },
        animation: { type: 'string', default: '' },  // Simple animation parameter
        animationClips: { type: 'array' }, // Array of animation clip names or objects
        autoPlay: { type: 'string', default: '' }, // Animation clip to auto-play
        loop: { type: 'boolean', default: true }, // Whether animations should loop
        timeScale: { type: 'number', default: 1.0 } // Animation playback speed
    },
    ...
    play: function () {
        ...
        if ((!this.added) &&
            ...
            // Set up animation clips after being added to the mesh
            setTimeout(() => {
                this.setupAnimationClips();
            }, 100); // Small delay to ensure mesh is fully initialized
        }
    },
    ...
    playAnimation: function (clipName, options = {}) {
        const mesh = this.data.mesh;
        if (mesh && mesh.components['instanced-mesh']) {
            const instancedMeshComponent = mesh.components['instanced-mesh'];
            const finalOptions = {
                loop: this.data.loop ? THREE.LoopRepeat : THREE.LoopOnce,
                timeScale: this.data.timeScale,
                ...options
            };
            instancedMeshComponent.playAnimation(this.el.id, clipName, finalOptions);
        }
    },

    stopAnimation: function (clipName = null) {
        const mesh = this.data.mesh;
        if (mesh && mesh.components['instanced-mesh']) {
            const instancedMeshComponent = mesh.components['instanced-mesh'];
            instancedMeshComponent.stopAnimation(this.el.id, clipName);
        }
    },

    // Set up animation clips from GLTF model
    setupAnimationClips: function () {
        if (!this.data.animationClips || this.data.animationClips.length === 0) return;

        const mesh = this.data.mesh;
        if (mesh && mesh.components['instanced-mesh']) {
            const instancedMeshComponent = mesh.components['instanced-mesh'];

            // Try to get clips from the mesh's model
            let availableClips = [];

            // Check if the original mesh has animations
            const originalMesh = instancedMeshComponent.originalMesh;
            if (originalMesh && originalMesh.animations) {
                availableClips = originalMesh.animations;
            }

            // Filter clips based on the specified names
            const selectedClips = availableClips.filter(clip =>
                this.data.animationClips.includes(clip.name)
            );

            if (selectedClips.length > 0) {
                instancedMeshComponent.initAnimationMixer(this.el, selectedClips);

                // Auto-play if specified
                if (this.data.autoPlay && selectedClips.find(clip => clip.name === this.data.autoPlay)) {
                    this.playAnimation(this.data.autoPlay);
                }
            }
        }
    },
