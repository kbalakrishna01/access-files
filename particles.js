(function () {
    ...
    const parseVecRange = (str, def) => {
        const parts = str.split('..').map(a => a.trim().split(' ').map((b) => {
            const num = Number(b)
    ...
    const parseVecRangeArray = (str, def) => flattenDeep(str.split(',').map(a => parseVecRange(a, def)))

    // parse a ("," separated) list of color range elements
    const parseColorRangeArray = str => flattenDeep(str.split(',').map((a) => {
        const parts = a.split('..')
        if (parts.length === 1) parts[1] = parts[0]  // if there is no second part then copy the first part
        parts.length = 2
    ...
    const WHITE_TEXTURE = new THREE.DataTexture(new Uint8Array(4).fill(255), 1, 1, THREE.RGBAFormat)
    ...
    AFRAME.registerComponent('sprite-particles', {
        ...
        update(oldData) {
            ...
            if (data.model !== oldData.model && data.model && 'getObject3D' in data.model) {
                if (oldData.model) {
                    oldData.model.removeEventListener('object3dset', this.handleObject3DSet)
                }
        ...
        updateColorOverTime() {
            const color = parseColorRangeArray(this.data.color)
            const opacity = parseVecRangeArray(this.data.opacity, [1])
        ...
        updateRotationScaleOverTime() {
            const maxSlots = this.data.overTimeSlots
            const rotation = parseVecRangeArray(this.data.rotation, [0])
            const scale = parseVecRangeArray(this.data.scale, [1])
            ...
            for (let i = 0, k = 2; i < n; i++, k += 2) {
                this.rotationScaleOverTime[k] = THREE.MathUtils.degToRad(rotation[i])
            }
        ...
        updateAngularVec4XYZRange(vecData, uniformAttr) {
            const vecRange = parseVecRange(vecData, [0, 0, 0])
            for (let i = 0, j = 0; i < vecRange.length;) {
                this[uniformAttr][j++] = THREE.MathUtils.degToRad(vecRange[i++])  // x
                this[uniformAttr][j++] = THREE.MathUtils.degToRad(vecRange[i++])  // y
                this[uniformAttr][j++] = THREE.MathUtils.degToRad(vecRange[i++])  // z
                j++  // skip the w
            }
        },
        ...
        updateAngularVec2PartRange(vecData, def, uniformAttr, part) {
            ...
            this[uniformAttr][part] = THREE.MathUtils.degToRad(vecRange[0])
            this[uniformAttr][part + 2] = THREE.MathUtils.degToRad(vecRange[1])
        },

        // update just the w component
        updateVec4WRange(floatData, def, uniformAttr) {
            const floatRange = parseVecRange(floatData, def)
        ...
        updateBounds() {
            ...
            const STRIDE = 4
            const extent = [new Array(STRIDE), new Array(STRIDE)]  // extent[0] = min values, extent[1] = max values

            if (data.drag > 0) {
                maxAge *= (1 - 0.5 * data.drag)
            }
        ...
        enableEditorObject(enable) {
            ...
            if (enable && (!existingMesh || existingMesh.isParticlesEditorObject)) {
                ...
                const box3 = new THREE.Box3(minBound, maxBound)
                const box3Mesh = new THREE.Box3Helper(box3, 0x808000)
        ...
        updateAttributes() {
            if (this.geometry) {
                ...
                const vertexIDs = new Float32Array(n)
        ...
        updateModelMesh(mesh) {
            ...
            const parseModel = (obj3D) => {
                ...
                const positions = obj3D.geometry.getAttribute('position')
        ...
        updateWorldTransform: (function () {
            ...
            return function (emitterTime) {
                ...
                const particleVertexID = this.geometry.getAttribute('vertexID')
                const particlePosition = this.geometry.getAttribute('position')
                const particleQuaternion = this.geometry.getAttribute('quaternion')
                ...
                if (numSpawned > 0) {
                    ...
                    this.nextID %= RANDOM_REPEAT_COUNT
                }
            }
        }()),
    })
